<?php
    ...
    $chk = $mysqli->prepare('SELECT post_no FROM community_posts_reports WHERE report_no = ? LIMIT 1');
    ...
    try {
        ...
        $stmt = $mysqli->prepare('UPDATE community_posts_reports SET status = ? WHERE report_no = ?');
        ...
        $stmt2 = $mysqli->prepare('UPDATE community_posts SET is_deleted = ? WHERE post_no = ?');
        $stmt2->bind_param('ii', $is_deleted, $post_no);
        if (!$stmt2->execute()) {
            throw new Exception('update post visibility failed');
        }
        $stmt2->close();
